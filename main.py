...
class UserVk:
    ...
    def get_photo(self, vk_id: str):
        ...
        photo_info = requests.get(photos_url, params={**self.params, **photos_params})
        if photo_info.status_code == 200:
            try:
                photo_info = photo_info.json()['response']
            except:
                print('Ой, что-то пошло не так! Проверьте исходные данные.')
                sys.exit()
        else:
            print('Ой, что-то пошло не так и мы не можем связаться с сервисами VK!')
            sys.exit()
...
class UserYandex:
    ...
    def __init__(self, token, num=10):
    ...
    def upload_file(self, dict_files: list, name_dir: str):
        ...
        if requests.get(url, headers=self.headers, params=params).status_code == 200:
            ...
            print('Ой, что-то пошло не так и мы не можем связаться с сервисами Яндекса!')
            sys.exit()
        in_folder_list = []
